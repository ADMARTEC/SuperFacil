...
namespace SuperFacil.Bussines.Servicos.Multimidia
    ...
    public class Mul_Midia_Servico : IMul_Midia_Servico
        ...
        public Mul_Midia GetTag(string Empresa, string value)
            {
            throw new NotImplementedException();
            }

        public void Save(string _Empresa, string _Parent, string _Designacao, string _Localizacao, string _Autor, int? _Faixa, bool Estado, string _User, bool _Activo, bool _Default = false)
            {
            var result = GetDesignacao(_Empresa, _Parent, _Designacao);

            if (result != null)
                {
                Update(_Empresa, _Parent, _Designacao, _Localizacao, _Autor, _Localizacao, _Autor, _Faixa, Estado, Estado, _User, _Activo, _Default);
                }
            else
                {
                Create(_Empresa, _Parent, _Designacao, _Localizacao, _Autor, _Localizacao, _Autor, _Faixa, Estado, Estado, _User, _Activo, _Default);
                }
            }

        public void Update(string _Empresa, string _Parent, string _Designacao, string _Localizacao, string _Autor, int? _Faixa, bool Estado, string _User, string _UpdateUser, bool _Activo, bool _Default = false)
            {
            var result = GetDesignacao(_Empresa, _Parent, _Designacao);

            result.Set_Midia(_Designacao, GetEmpresa(_Empresa), GetParent(_Empresa, _Parent), GetLocalizacao(_Empresa, _Localizacao), GetAutor(_Empresa, _Autor), _Faixa, Estado, _User);
            result.Set_Cud_Update(_Activo, GetUsuario(_Empresa, _UpdateUser), DateTime.Now, _Default);

            _repositorio.Update(result);
            }

        public void Save(string _Empresa, string _Parent, string _Classificacao, string _Genero, string _Autor, DateTime _DataLancamento, string _Editora, int? _Faixa, TimeSpan _Duracao, string _Tag, bool Estado, string _User, bool _Activo, bool _Default = false)
            {
            throw new NotImplementedException();
            }

        public void Delete(string _Empresa, string _Parent, string _Classificacao, string _Genero, int? _Faixa, TimeSpan _Duracao, string _Editora, string _DeletedUser)
            {
            throw new NotImplementedException();
            }
        }
